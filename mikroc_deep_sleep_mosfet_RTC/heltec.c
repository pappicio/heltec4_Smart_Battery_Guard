/*
 * Programma: Solar_Power_Manager_683_Final_V12_RTC
 * Traduzione in mikroC PRO for PIC
 */

// --- CONFIGURAZIONE PIN I2C SOFTWARE ---
sbit Soft_I2C_Scl           at GP0_bit;
sbit Soft_I2C_Sda           at GP2_bit;
sbit Soft_I2C_Scl_Direction at TRISIO0_bit;
sbit Soft_I2C_Sda_Direction at TRISIO2_bit;

// --- VARIABILI GLOBALI ---
bit RSTpin;
bit acceso;
bit RTC_presente;
bit finestra_oraria;
bit spento;
bit in_manutenzione;
bit reset_fatto;

unsigned int batteria_mv;      // Memorizza la tensione batteria in millivolt
unsigned short i, j;           // Variabili di utilità
unsigned int sveglie_wdt;      // Conta quante volte il Watchdog ha svegliato il chip
unsigned short dummy;          // Appoggio per lettura GPIO
unsigned int soglia_off, soglia_on;
unsigned int taratura_vcc;
unsigned int val_da_lampeggiare;

// --- VARIABILI RTC E CONVERSIONE ---
unsigned short ore, minuti, giorno;
unsigned short minuti_count;

// --- VARIABILI TIMER RIAVVIO ---
unsigned short giorni_riavvio;
unsigned long conteggio_cicli;
unsigned int cicli_per_giorno;

// --- RITARDO SICURO (Mantiene il Watchdog pulito) ---
void Delay_Safe_ms(unsigned int n) {
    unsigned int k;
    for (k = 0; k < n; k++) {
        Delay_us(978);
        asm clrwdt; // Reset Watchdog in Assembly per mikroC
    }
}

// --- SUBROUTINE LAMPEGGIO CIFRA ---
void Lampeggia_Cifra(unsigned short c) {
    unsigned short l;
    if (c == 0) {
        // Zero: lampeggio brevissimo
        GP5_bit = 1;
        Delay_Safe_ms(50);
        GP5_bit = 0;
    } else {
        for (l = 0; l < c; l++) {
            GP5_bit = 1;
            Delay_Safe_ms(250);
            GP5_bit = 0;
            Delay_Safe_ms(250);
            asm clrwdt;
        }
    }
    Delay_Safe_ms(1000);
}

// --- ESTRAZIONE CIFRE ---
void Estrai_e_Lampeggia(unsigned int divisore) {
    unsigned short contatore = 0;
    while (val_da_lampeggiare >= divisore) {
        val_da_lampeggiare -= divisore;
        contatore++;
    }
    Lampeggia_Cifra(contatore);
}

// --- LETTURA ORA RTC ---
void Leggi_Ora_RTC() {
    unsigned short bcd_temp;

    GP5_bit = 1;
    Delay_Safe_ms(100);

    Soft_I2C_Start();
    Soft_I2C_Stop();
    Delay_Safe_ms(10);

    // Minuti
    Soft_I2C_Start();
    Soft_I2C_Write(0xD0);
    Soft_I2C_Write(0x01);
    Soft_I2C_Start();
    Soft_I2C_Write(0xD1);
    bcd_temp = Soft_I2C_Read(0);
    Soft_I2C_Stop();
    minuti = ((bcd_temp >> 4) * 10) + (bcd_temp & 0x0F);

    Delay_Safe_ms(10);

    // Ore
    Soft_I2C_Start();
    Soft_I2C_Write(0xD0);
    Soft_I2C_Write(0x02);
    Soft_I2C_Start();
    Soft_I2C_Write(0xD1);
    bcd_temp = Soft_I2C_Read(0);
    Soft_I2C_Stop();
    bcd_temp &= 0x3F;
    ore = ((bcd_temp >> 4) * 10) + (bcd_temp & 0x0F);

    GP5_bit = 0;
}

// --- LETTURA ANALOGICA E MEDIA ---
void Leggi_Batteria_mV() {
    unsigned short k;
    unsigned long somma = 0;
    unsigned int media_pulita;

    for (k = 0; k < 64; k++) {
        somma += ADC_Read(1);
        Delay_Safe_ms(1);
    }
    media_pulita = (unsigned int)(somma >> 6);
    batteria_mv = (unsigned int)(( (unsigned long)media_pulita * taratura_vcc ) >> 10);
}

// --- UNIFICA SEGNALI VISIVI ---
void Lampi(unsigned short n, unsigned int t_on) {
    unsigned short k;
    for (k = 0; k < n; k++) {
        GP5_bit = 1;
        Delay_Safe_ms(t_on);
        GP5_bit = 0;
        Delay_Safe_ms(t_on);
    }
}

// --- FEEDBACK VISIVO STATO BATTERIA ---
void soglia_batteria() {
    if (batteria_mv <= soglia_off) {
        GP5_bit = 0;
        Delay_Safe_ms(500);
        Lampi(6, 100);
    } else if (batteria_mv > soglia_off && batteria_mv <= soglia_on) {
        Delay_Safe_ms(500);
        Lampi(3, 100);
    }
}

// --- SCRITTURA ORA RTC ---
void Scrivi_Ora_RTC(short s_g_sett, short s_g, short s_m, short s_a, short s_ore, short s_min) {
    GP5_bit = 1;
    Delay_Safe_ms(100);
    Soft_I2C_Init();
    Delay_Safe_ms(100);
    Soft_I2C_Start();
    Soft_I2C_Write(0xD0);
    Soft_I2C_Write(0x00); // Registro secondi
    Soft_I2C_Write(0x00);
    Soft_I2C_Write(s_min);
    Soft_I2C_Write(s_ore);
    Soft_I2C_Write(s_g_sett);
    Soft_I2C_Write(s_g);
    Soft_I2C_Write(s_m);
    Soft_I2C_Write(s_a);
    Soft_I2C_Stop();
    Delay_Safe_ms(800);
    GP5_bit = 0;
    Delay_Safe_ms(500);
}

// --- INIZIALIZZAZIONE ---
void Init_Hardware() {
    OSCCON = 0b01100111;    // 4MHz
    CMCON0 = 7;             // No comparatori
    ANSEL  = 0b00010010;    // AN1 Analogico
    TRISIO = 0b00001010;    // GP1, GP3 Input
    OPTION_REG = 0b00001111; // WDT 1:128
    WPU = 0;
    INTCON.GPIE = 1;
    IOC.B3 = 1;             // Interrupt on change GP3

    conteggio_cicli = 0;
    cicli_per_giorno = 2883;
    spento = 0;
    acceso = 1;
    RSTpin = 1;

    RTC_presente = 1;
    finestra_oraria = 0;
    giorni_riavvio = 0;

    soglia_off   = 3300;
    soglia_on    = 3600;
    taratura_vcc = 5010;

    if (RSTpin) acceso = 0;

    GP4_bit = acceso;       // Mosfet
    GP5_bit = 0;            // LED

    if (RTC_presente) {
        TRISIO0_bit = 0;
        TRISIO2_bit = 0;
        GP0_bit = 1;
        GP2_bit = 1;

        i = 0;
        while (GP3_bit == 0 && i < 15) {
            GP5_bit = 1;
            Delay_Safe_ms(100);
            i++;
        }
        if (i == 15) {
            GP5_bit = 0;
            // Esempio: Sabato 04/04/26 ore 20:55
            Scrivi_Ora_RTC(0x06, 0x04, 0x04, 0x26, 0x20, 0x55);
            Lampi(10, 100);
        }
    } else {
        TRISIO0_bit = 1;
        TRISIO2_bit = 1;
    }

    Delay_Safe_ms(500);
    Lampi(3, 250);
    Delay_Safe_ms(500);
    Leggi_Batteria_mV();

    if (batteria_mv > soglia_off) {
        GP4_bit = !acceso;
        spento = 0;
    } else {
        spento = 1;
    }

    in_manutenzione = 0;
    reset_fatto = 0;
    sveglie_wdt = 0;
    soglia_batteria();
}

// --- MAIN ---
void main() {
    Init_Hardware();

    while (1) {
        if (INTCON.GPIF) {
            dummy = GPIO;
            INTCON.GPIF = 0;
        }

        if (GP3_bit == 0) {
            i = 0;
            while (GP3_bit == 0 && i < 50) {
                Delay_Safe_ms(100);
                i++;
                if (i == 10) GP5_bit = 1;
                if (i == 25) GP5_bit = 0;
            }

            // 1. Reset Rapido (1-2.5s)
            if (i >= 10 && i < 25) {
                GP5_bit = 0;
                Leggi_Batteria_mV();
                GP4_bit = acceso;
                Delay_Safe_ms(2000);
                if (batteria_mv > soglia_off) {
                    GP4_bit = !acceso;
                    spento = 0;
                } else {
                    spento = 1;
                }
                if (batteria_mv < soglia_on) soglia_batteria();
                sveglie_wdt = 0;
                conteggio_cicli = 0;
            }

            // 2. Visualizzazione Volt (2.5-5s)
            if (i >= 25 && i < 50) {
                GP5_bit = 0;
                Leggi_Batteria_mV();
                Delay_Safe_ms(1000);
                val_da_lampeggiare = batteria_mv;
                Estrai_e_Lampeggia(1000);
                Estrai_e_Lampeggia(100);
                Estrai_e_Lampeggia(10);
                Lampeggia_Cifra(0);

                if (RTC_presente) {
                    Delay_Safe_ms(1000);
                    Lampi(2, 100);
                    Leggi_Ora_RTC();
                    val_da_lampeggiare = ore;
                    Estrai_e_Lampeggia(10);
                    Lampeggia_Cifra((unsigned short)val_da_lampeggiare);
                    Delay_Safe_ms(1000);
                    val_da_lampeggiare = minuti;
                    Estrai_e_Lampeggia(10);
                    Lampeggia_Cifra((unsigned short)val_da_lampeggiare);
                }
            }

            // 3. Manutenzione (>5s)
            if (i >= 50) {
                GP4_bit = acceso;
                for (j = 0; j < 20; j++) {
                    GP5_bit = !GP5_bit;
                    Delay_Safe_ms(100);
                }
                GP5_bit = 0;
                in_manutenzione = 1;
                while (in_manutenzione) {
                    GP5_bit = 1;
                    Delay_Safe_ms(500);
                    GP5_bit = 0;
                    if (GP3_bit == 0) {
                        i = 0;
                        while (GP3_bit == 0 && i < 50) {
                            Delay_Safe_ms(100);
                            i++;
                        }
                        if (i >= 50) {
                            in_manutenzione = 0;
                            for (j = 0; j < 20; j++) {
                                GP5_bit = !GP5_bit;
                                Delay_Safe_ms(100);
                            }
                        }
                    } else {
                        Delay_Safe_ms(500);
                    }
                    asm clrwdt;
                }
                Leggi_Batteria_mV();
                if (batteria_mv > soglia_off) {
                    GP4_bit = !acceso;
                    spento = 0;
                } else {
                    spento = 1;
                }
                if (batteria_mv < soglia_on) soglia_batteria();
                sveglie_wdt = 13;
                conteggio_cicli = 0;
                minuti_count = 0;
            }
        }

        if (!in_manutenzione) {
            if (sveglie_wdt >= 13) {
                Leggi_Batteria_mV();
                if (batteria_mv <= soglia_off) {
                    GP4_bit = acceso;
                    spento = 1;
                }
                if (batteria_mv >= soglia_on) {
                    GP4_bit = !acceso;
                    spento = 0;
                }
                sveglie_wdt = 0;

                if (RTC_presente) {
                    giorni_riavvio = 0;
                    minuti_count++;
                } else {
                    minuti_count = 0;
                    finestra_oraria = 0;
                }

                if (giorni_riavvio > 0) {
                    conteggio_cicli++;
                    if (conteggio_cicli >= (unsigned long)cicli_per_giorno * giorni_riavvio) {
                        GP4_bit = acceso;
                        Delay_Safe_ms(2000);
                        if (batteria_mv > soglia_off) {
                            GP4_bit = !acceso;
                            spento = 0;
                        }
                        conteggio_cicli = 0;
                    }
                }

                if (minuti_count >= 20) {
                    Leggi_Ora_RTC();
                    if (finestra_oraria == 0) {
                        if (ore == 4) {
                            if (reset_fatto == 0) {
                                if (giorno == 1 || giorno == 4) {
                                    GP4_bit = acceso;
                                    Delay_Safe_ms(10000);
                                    if (batteria_mv > soglia_off && spento == 0) GP4_bit = !acceso;
                                    reset_fatto = 1;
                                }
                            }
                        } else {
                            reset_fatto = 0;
                        }
                    } else {
                        if (ore >= 7 && ore < 13) {
                            if (batteria_mv > soglia_off && spento == 0) GP4_bit = !acceso;
                            else GP4_bit = acceso;
                        } else {
                            GP4_bit = acceso;
                        }
                    }
                    minuti_count = 0;
                }
            }
            sveglie_wdt++;
            asm clrwdt;
            asm sleep;
            asm nop;
        } else {
            Delay_Safe_ms(100);
            asm clrwdt;
        }
    }
}